...
Returns a string, object, array of strings, or array of objects depending on the context.
{/* Why does this script's output vary so much?? */}

#### CLI

When called from the CLI with no arguments, returns a string:
...
When called from the CLI with any arguments, returns an array of strings:

```
>>sys.access_log literally anything
240116.2216 Lock rotation. Breach state reset.
240116.2114 Lock rotation. Breach state reset.
240116.2012 Lock rotation. Breach state reset.
240116.1910 Lock rotation. Breach state reset.
...
When called as a scriptor with null or falsey input, returns an object.

```
{
  msg: "To filter list, add user:"<username>" or run_id:"run_id"\nGet a range with count:<count> and start:<offset>",
  logs: [
    "260804.0148 Lock rotation. Breach state reset.",
    "260804.0030 Lock rotation. Breach state reset.",
    "260803.2328 Lock rotation. Breach state reset.",
    "260803.2226 Lock rotation. Breach state reset.",
    "260803.2124 Lock rotation. Breach state reset.",
    "260803.2022 Lock rotation. Breach state reset.",
    "260803.1920 Lock rotation. Breach state reset.",
    "260803.1818 Lock rotation. Breach state reset.",
    "260803.1716 Lock rotation. Breach state reset.",
    "260803.1614 Lock rotation. Breach state reset."
  ]
}
```

When called as a scriptor with non-falsey input, returns an array of strings.
...
When called as a subscript, returns an array of objects.
