...
Causes a parse error when present in a script.

```
>>#up example
Failure
PARSE ERROR sys.init: script cannot be included in another script.
```
...
Returns a script response object, or a string.
...
Cannot be called as a subscript or scriptor.
